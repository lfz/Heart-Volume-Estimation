...
def build_fcn(input_var, inner_size):
    ...
    # top-down stage 0
    l4_conv = batch_norm(Conv2DLayer(conv4_2, num_filters=4, filter_size=(1, 1),nonlinearity=rectify, W=HeNormal()))
    up4 = Upscale2DLayer(l4_conv, (8, 8))

    
    l3_conv = batch_norm(Conv2DLayer(conv3_2, num_filters=4, filter_size=(1, 1),nonlinearity=rectify, W=HeNormal()))
    up3 = Upscale2DLayer(l3_conv, (4, 4))

    l2_conv = batch_norm(Conv2DLayer(conv2_3, num_filters=4, filter_size=(1, 1),nonlinearity=rectify, W=HeNormal()))
    up2 = Upscale2DLayer(l2_conv, (2, 2))

    
    l1_conv = batch_norm(Conv2DLayer(conv1_3, num_filters=4, filter_size=(1, 1),nonlinearity=rectify, W=HeNormal()))

    concat = ConcatLayer([up4,up3,up2,l1_conv])
    

    pred = Conv2DLayer(concat, num_filters=2, filter_size=(3, 3),nonlinearity=my_softmax, W=HeNormal(),pad=1)
    concat2 = ConcatLayer([concat,pred])
    area1 = Conv2DLayer(concat2, num_filters=8, filter_size=(3, 3),nonlinearity=rectify, W=HeNormal(),pad=1)
